...
class StudentMicroservice {
    ...
    private fun subscribeToHearbeat() {
        try {
            ...
            heartbeatSocket.getOutputStream().write(("student\n").toByteArray())
    ...
    public fun run() = runBlocking {
        ...
        launch(Dispatchers.IO) {
            ...
            while (true) {
                ...
                println("intrebarea este: ${response}")

                // se foloseste un thread separat pentru tratarea intrebarii primite
                launch(Dispatchers.IO) {
                    println("am intrat in launch cu raspunsul ${response}")
        ...
        launch(Dispatchers.IO) {
            val buffer = BufferedReader(InputStreamReader(heartbeatSocket.inputStream))
            while(true){
                val msg=buffer.readLine()
                heartbeatSocket.getOutputStream().write(("student\n").toByteArray())
            }
