...
class BiddingProcessorMicroservice {
    ...
    init {
        ...
        taskManagerSocket=Socket(AUCTIONEER_HOST, TASK_MANAGER_PORT)
        var startMessage=Message.create(biddingProcessorSocket.localSocketAddress.toString(),"Am inceput treaba","BiddingProcessor")
        ...
        val receiveProcessedBidsSubscription = receiveProcessedBidsObservable
            .subscribeBy(
                ...
                onComplete = {
                    ...
                    taskManagerSocket=Socket(AUCTIONEER_HOST, TASK_MANAGER_PORT)
                    var startMessage=Message.create(biddingProcessorSocket.localSocketAddress.toString(),"Am primit ${processedBidsQueue.size}","BiddingProcessor")
                    taskManagerSocket.getOutputStream().write(startMessage.serialize())
                    taskManagerSocket.close()
                    decideAuctionWinner()
                },
    ...
    private fun decideAuctionWinner() {
        ...
        try {
            ...
            taskManagerSocket=Socket(AUCTIONEER_HOST, TASK_MANAGER_PORT)
            var startMessage=Message.create(biddingProcessorSocket.localSocketAddress.toString(),"Am terminat treaba","BiddingProcessor")
            taskManagerSocket.getOutputStream().write(startMessage.serialize())
            taskManagerSocket.close()
        } catch (e: Exception) {
