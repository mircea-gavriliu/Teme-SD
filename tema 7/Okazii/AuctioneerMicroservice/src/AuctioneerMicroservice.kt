...
class AuctioneerMicroservice {
    ...
    init {
        ...
        taskManagerSocket=Socket(MESSAGE_PROCESSOR_HOST, TASK_MANAGER_PORT)
        var startMessage=Message.create(auctioneerSocket.localSocketAddress.toString(),"Am inceput treaba","AuctioneerMicroservice")
    ...
    private fun receiveBids() {
        ...
        val receiveBidsSubscription = receiveBidsObservable.subscribeBy(
            ...
            onComplete = {
                ...
                taskManagerSocket=Socket(MESSAGE_PROCESSOR_HOST, TASK_MANAGER_PORT)
                var startMessage=Message.create(auctioneerSocket.localSocketAddress.toString(),"Am primit in total ${bidQueue.size}","AuctioneerMicroservice")
                taskManagerSocket.getOutputStream().write(startMessage.serialize())
                taskManagerSocket.close()
                forwardBids()
            },
    ...
    private fun finishAuction() {
        ...
        try {
            ...
            taskManagerSocket=Socket(MESSAGE_PROCESSOR_HOST, TASK_MANAGER_PORT)
            var startMessage=Message.create(auctioneerSocket.localSocketAddress.toString(),"Am  terminat treaba","AuctioneerMicroservice")
            taskManagerSocket.getOutputStream().write(startMessage.serialize())
            taskManagerSocket.close()
        } catch (e: Exception) {
