...
class MessageProcessorMicroservice {
    ...
    init {
        ...
        taskManagerSocket=Socket(BIDDING_PROCESSOR_HOST, TASK_MANAGER_PORT)
        var startMessage=Message.create(messageProcessorSocket.localSocketAddress.toString(),"Am inceput treaba","MessageProcessorMicroservice")
        taskManagerSocket.getOutputStream().write(startMessage.serialize())
        ...
        val receiveInQueueSubscription = receiveInQueueObservable
            ...
            .subscribeBy(
                onNext = {
                    ...
                    println(message)
                    if(messageQueue.filter{ it.sender.equals(message.sender)}.isEmpty())
                        messageQueue.add(message)
                },
                onComplete = {
                    // s-a incheiat primirea tuturor mesajelor
                    ///TODO --- se ordoneaza in functie de data si ora cand mesajele au fost primite
                    messageQueue= messageQueue.sortedBy { it.timestamp }.toMutableList()
                    ...
                    taskManagerSocket=Socket(BIDDING_PROCESSOR_HOST, TASK_MANAGER_PORT)
                    var startMessage=Message.create(messageProcessorSocket.localSocketAddress.toString(),"Am primit si procesat ${messageQueue.size}","MessageProcessorMicroservice")
                    taskManagerSocket.getOutputStream().write(startMessage.serialize())
                    taskManagerSocket.close()
        ...
        try {
            ...
            Observable.fromIterable(messageQueue).subscribeBy(
                ...
                onComplete = {
                    ...
                    taskManagerSocket=Socket(BIDDING_PROCESSOR_HOST, TASK_MANAGER_PORT)
                    var startMessage=Message.create(messageProcessorSocket.localSocketAddress.toString(),"Am terminat treaba","MessageProcessorMicroservice")
                    taskManagerSocket.getOutputStream().write(startMessage.serialize())
                    taskManagerSocket.close()
